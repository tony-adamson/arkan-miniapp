...
**18.09.2026 — /verify после O31**: ожидания сделаны конечными — `lock_timeout` на соединениях api (`DB_LOCK_TIMEOUT_MS`, таймаут блокировки → 409) и таймауты сокета Redis, без которых fail-open §9.6 не наступал; `SESSION_SECRET` не короче 16 символов, иначе подпись cookie вычислима публично.
...
**18.09.2026 — поправка по итогам фазы 6 (пятая)**: форма ответов вынесена в отдельный §7.1 и описана целиком для существующих эндпоинтов (позиция, расклад, строка истории, отказ, кризис, ошибка). Причина: верификатор четыре круга подряд валил фазу на полях, которых не было ни в одном контракте (`revealed`, `verify_status`, `category`, `structure_type`, `revealed_at`, `ask_variant`, `ask_answer`), а я латал формулировку по одному полю. Фазам разрешено дописывать §7.1 перечислением своих полей — это снимает остановку на каждом новом объекте ответа.
