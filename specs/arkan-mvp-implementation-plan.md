...
- формат ошибок: `{"error": "<code>", "message": "<текст в тоне GUIDE>"}`; коды — закрытый перечень SOLUTION §9.5 (D21): `unauthorized`, `forbidden`, `not_found`, `conflict`, `validation`, `rate_limited`, `llm_unavailable`; новый код вводится только правкой §9.5;
- форма позиции в ответах: у каждой позиции всегда есть `position_number`, `position_name` и булев `revealed`; поля `card_id`, `interpretation`, `verify_status`, вопрос и варианты приходят только у раскрытых (`revealed: true`). Флаг карту не раскрывает, поэтому запрет «только номер и название» им не нарушается;
