...
D20 (сервер в РФ, Telegram через `TELEGRAM_PROXY`). Порядок расклада и
...
- формат ошибок: `{"error": "<code>", "message": "<текст в тоне GUIDE>"}`;
...
4. `app/expert/base.py`: загрузка YAML; `get_card`, `get_spread_for_category` (неизвестная категория → `choice`, комментарий ASM-7), `get_refusal`, `get_crisis_resources`, `get_checkin_text(variant)`, `position_question(position, variant)` (пустой `ask_2` → `ask_1` с его вариантами).
5. `make validate-base CSV=<dir> OUT=<dir>`.
